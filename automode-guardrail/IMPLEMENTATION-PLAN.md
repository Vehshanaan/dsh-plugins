...
- **输出协议**：第一行 `allow|deny`，第二行类别 token（allow 必须配 `safe`；deny 配六个风险类别之一），其余为理由（截断 300 字符）。解析失败 = 拒绝。
- **工程保险**：`ctx.llm.stream` + `deadline(timeoutMs)`；超时/服务商错误/中止/格式非法 → 拒绝，理由注明"classifier unavailable"，**绝不默认放行**。
